...
public class ShopController : MonoBehaviour
{
    ...
    void Start()
    {
       ...
       // uIManager = FindAnyObjectByType<UIManager>();
        ...
        //moneyText.text = p.coinNum.ToString();
        //moneyText.text = SaveSystem.LoadMoney().money.ToString(); //Only if you have a text for your money
    ...
    public void StartShop()
    {
        ...
        foreach (Item item in items)
        {
            ...
            card.transform.Find("Price").GetComponent<TextMeshProUGUI>().text = item.price + "G";
    ...
    public void Buy(Item item)
    {
        ...
        if (item.price <= actualMoney && item.finalAmount > 0)
        {
            ...
            //item.i refers to the actual InventoryPlus item to add to the inventory
            //p.inventory.AddInventory(item.i, 1, 0, true);
        }
        else
        {
            //Don't buy
